...
import { Container, Main, RightSide, FieldArea } from "./style";
...
const FieldJournal: React.FC = () => {
    ...
    const getFieldJournal = async (
        moderador_id: string,
        encontro_id: string
    ) => {
        const response = await api.get(
            `diarios?moderador_id=${moderador_id}&encontro_id=${encontro_id}`,
            {
                headers: {
                    Authorization: auth,
                },
    ...
    const updateFieldJournal = async (
        fieldJournal_id: string,
        value: fieldJournal
    ) => {
                ...
                <RightSide>
                    ...
                    <FieldArea>
                        ...
                        <Button
                            ...
                            onClick={() => {
                                try {
                                    const [id] = fieldJournal.map(
                                        (fj) => fj["id"]
                                    );
                                    const value = {
                                        description: textAreaValue || "",
                                    };
                                    updateFieldJournal(id, value);
                                    alert("Salvo!");
                                } catch (err) {
                                    alert(`Não salvo!`);
                                    console.log(err);
